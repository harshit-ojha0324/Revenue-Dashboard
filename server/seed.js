...
// Shared sample-data helpers (kept in one place so the CLI seeder and the live
// node-cron seeder generate consistent data).
const { buildRandomSale, getRandomDate } = require('./utils/sampleData');
...
const generateSalesData = async (userId, numSales = 50) => {
  ...
  // orderId is auto-generated by the Sale model's default.
  const salesData = Array.from({ length: numSales }).map(() =>
    buildRandomSale(userId, getRandomDate(oneYearAgo, today))
  );
