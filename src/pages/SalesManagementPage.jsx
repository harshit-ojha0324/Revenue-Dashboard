...
const SalesManagementPage = () => {
  ...
  const goToPage = (target) => { setPage(target); load(target); };
  ...
  return (
    <>
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-semibold text-gray-900 dark:text-white">Sales Management</h1>
          <p className="text-gray-600 dark:text-gray-400">Every sale across all users</p>
        </div>
          ...
          <table className="min-w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {['Order ID', 'Product', 'Customer', 'Category', 'Date', 'Qty', 'Total', 'Actions'].map((h) => (
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {isLoading ? (
                ...
                <tr key={sale._id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td className="py-4 px-6 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{sale.customer?.name || '—'}</td>
                ...
                <tr><td colSpan="8" className="py-8 px-6 text-center text-sm text-gray-500 dark:text-gray-400">No sales found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {total > 0 && (
          <div className="px-6 py-3 flex items-center justify-between border-t border-gray-200 dark:border-gray-700">
            ...
            <div className="flex gap-2">
              <button onClick={() => goToPage(page - 1)} disabled={page <= 1 || isLoading} className="px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md disabled:opacity-50 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">Previous</button>
              <button onClick={() => goToPage(page + 1)} disabled={page >= totalPages || isLoading} className="px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md disabled:opacity-50 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">Next</button>
            </div>
          </div>
        )}
      </div>
      ...
      {modalOpen && (
        <SaleFormModal sale={editingSale} customers={users} onClose={closeModal} onSubmit={handleSubmit} isSubmitting={submitting} />
      )}
    </>
  );
};
